...
@implementation SVPDes

+ (NSString *) encode:(NSString *)str key:(NSString *)key
{
    // doCipher 不能编汉字，所以要进行 url encode
    NSMutableString* str1 = [SVPDes urlEncode:str];
    NSMutableString* encode = [NSMutableString stringWithString:[SVPDes doCipher:str1 key:key context:kCCEncrypt]];
...
+ (NSString *) decode:(NSString *)str key:(NSString *)key
{
    ...
    NSString *rt = [SVPDes doCipher:str1 key:key context:kCCDecrypt];
...
+ (NSString *)encodeBase64WithString:(NSString *)strData {
    return [SVPDes encodeBase64WithData:[strData dataUsingEncoding:NSUTF8StringEncoding]];
}
...
+ (NSString *)doCipher:(NSString *)sTextIn key:(NSString *)sKey context:(CCOperation)encryptOrDecrypt {
    ...
    if (encryptOrDecrypt == kCCDecrypt) {
        dTextIn = [[SVPDes decodeBase64WithString:sTextIn] mutableCopy];
    } else {
    ...
    if (encryptOrDecrypt == kCCDecrypt){
        ...
        free(bufferPtr1);
        sResult = [SVPDes encodeBase64WithData:dResult];
    }
